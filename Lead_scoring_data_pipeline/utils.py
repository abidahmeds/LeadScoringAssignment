...
# import libraries from airflow Dags folders 
...
# some helper functions to load data from CSV and checking if tables are present and have values 
...
def build_dbs():
    ...
    if os.path.isfile(DB_PATH + DB_FILE_NAME):
        ...
        try:
            ...
            print(e)
            return "There is a Error creating DB " + DB_PATH + DB_FILE_NAME
...
def load_data_into_db():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'loaded_data'):
            print("Loading data from " + LEADSCORING_CSV_DATA_PATH)
            df = load_data(LEADSCORING_CSV_DATA_PATH)
        ...
        print (f'Some exception thrown in load_data_into_db : {e}')
    finally:
        if cnx:        
            cnx.close()
...
def interactions_mapping():
    ...
    try:
        ...
        if not check_if_table_has_value(cnx, 'model_input') or not check_if_table_has_value(cnx, 'interactions_mapped'):
            ...
            df_event_mapping = pd.read_csv(INTERACTION_MAPPING_INFO, index_col=[0])

            # unpivot the interaction columns and put the values in rows
            id_vars = INDEX_OF_COLUMNS_NEEDED
            ...
            print("Selecting a subset of columns for model traning part, excluding created_date")
            # these columns were derived after rapid expermentation where we excluded columns with relatively low significance
            dataset_trimmed = df_pivot[INDEX_OF_COLUMNS_NEEDED[1:]]
            ...
            print('model_input and interactions_mapped already populated')
    except Exception as e:
